...
//  This version of the program writes the blank spaces in the blank rows all at once, instead of
		...
		while (c != column)
		{
			...
			if (r == pad + 1 && c == pad + 1)
			{
				...
				if (r == 0 || r == row - 1 || c == 0 || c == column - 1) {
					...
					// Write blanks up until last "*" at the end of row, unless it's the greeting row
					if (r != pad + 1)
					{
						std::string blank_string(greeting.size() + pad * 2, ' ');
						std::cout << blank_string;
						c += greeting.size() + pad * 2;
					}
					else {
						// Write one blank character if it's the greeting row
						std::cout << " ";
						c++;
					}
				}
